    ...
    <div>
      ...
      <div className="table-responsive">
        {timeSlotList && timeSlotList.length > 0 ? (
          <Table responsive>
            <thead>
              <tr>
                ...
                <th>
                  Pack <FontAwesomeIcon icon="sort" />
                </th>
                <th />
              </tr>
            </thead>
            <tbody>
              {timeSlotList.map((timeSlot, i) => (
                <tr key={`entity-${i}`} data-cy="entityTable">
                  ...
                  <td>{timeSlot.status ? 'true' : 'false'}</td>
                  <td>{timeSlot.doctor ? <Link to={`/doctor/${timeSlot.doctor.id}`}>{timeSlot.doctor.id}</Link> : ''}</td>
                  <td>{timeSlot.pack ? <Link to={`/pack/${timeSlot.pack.id}`}>{timeSlot.pack.id}</Link> : ''}</td>
                  <td className="text-end">
                    <div className="btn-group flex-btn-group-container">
                      ...
                      <Button
                        tag={Link}
                        to={`/time-slot/${timeSlot.id}/delete?page=${paginationState.activePage}&sort=${paginationState.sort},${paginationState.order}`}
                        color="danger"
                        size="sm"
          ...
          !loading && <div className="alert alert-warning">No Time Slots found</div>
        )}
      </div>
      {totalItems ? (
